/**
 * 手搖飲日記 - Google Apps Script 後端 API
 * 
 * 功能：
 * - GET: 讀取所有記錄
 * - POST: 新增、編輯、刪除記錄
 ...
 * 2. 替換 SPREADSHEET_ID 為您的試算表 ID
 * 3. 部署為網頁應用程式
 * 4. 設定「具有存取權的使用者」為「任何人」
 */

// ⚠️ 重要：請替換為您的 Google Sheets 試算表 ID
// 試算表 ID 可以在網址中找到：https://docs.google.com/spreadsheets/d/你的試算表ID/edit
...
function doPost(e) {
  try {
    ...
    if (!sheet) {
      ...
      // 設定標題列
      newSheet.getRange(1, 1, 1, 5).setValues([['日期', '店家', '飲料', '價格', 'ID']]);
      return ContentService.createTextOutput(JSON.stringify({ success: true, message: '工作表已建立' }))
        .setMimeType(ContentService.MimeType.JSON);
    }
